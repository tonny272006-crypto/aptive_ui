...
class FlashcardDeck {
  ...
  FlashcardDeck({required this.title, required this.cardImageUrls})
    : cardBioRate = List<int>.filled(cardImageUrls.length, 0);
}

// --- 2. THE MAIN WIDGET (Handles Vertical Deck Swiping) ---
class AptivReviewPage extends StatefulWidget {
  final String data;
  const AptivReviewPage({super.key, required this.data});

  @override
  State<AptivReviewPage> createState() => _AptivReviewPageState();
}

class _AptivReviewPageState extends State<AptivReviewPage> {
   ...
   */
  final List<FlashcardDeck> _decksbio = [
    FlashcardDeck(
  ...
  final List<FlashcardDeck> _deckschem = [
    FlashcardDeck(
  ...
  final List<FlashcardDeck> _decksphy = [
    FlashcardDeck(
  ...
  final List<FlashcardDeck> _deckshis = [
    FlashcardDeck(
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        margin: const EdgeInsets.symmetric(vertical: 0.10),
